<%@ page language="java" contentType="application/json;" pageEncoding="UTF-8" %>
<%@page import="java.util.List"%>
<%@page import="java.util.HashMap"%>
<%@page import="startkr.util.RequestUtils"%>
<%@page import="startkr.util.JsonUtils"%>
<%@page import="org.json.simple.JSONArray"%>
<%@page import="com.cofac.treat.ora.biz.non.TreatBiz"%>
<%@page import="com.cofac.treat.ora.biz.non.RoomBiz"%>
<%
RequestUtils util = new RequestUtils();
HashMap paramMap = util.makeParamMap(request);

List list = null;
TreatBiz treatBiz = null;
RoomBiz roomBiz = null;
try {
	treatBiz = new TreatBiz();
  list = treatBiz.selectTreatList(paramMap);
  JSONArray jsonTreatArr = new JSONArray();
  jsonTreatArr = JsonUtils.getJsonArrayFromList(list);
  
  paramMap.put("use_fg", "Y");
  
  roomBiz = new RoomBiz();
  paramMap.put("maxResults", 7);
  list = roomBiz.selectRoomList(paramMap);
  JSONArray jsonRoomArr = new JSONArray();
  jsonRoomArr = JsonUtils.getJsonArrayFromList(list);
    
  out.print("{\"resultTreat\":"+jsonTreatArr+",\"resultRoom\":"+jsonRoomArr+"}");
} catch(Exception ex) {
    ex.printStackTrace();
}
%>