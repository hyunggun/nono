...
// 	treatBiz = new OratreatBiz();
	treatBiz = new OratreatBiz();
	resultMap = treatBiz.selectTreatCheckPage(paramMap);
} catch(Exception ex) {
    ex.printStackTrace();
}
%>
	                ...
	                <div class="email-search">
	                  <div class="input-group input-search input-group-sm cf-mobile-margin">
	                    <select id="search_type" name="search_type" class="form-control input-sm">
	                    	...
	                    	<option value="51">검사</option>
	                    	<option value="70">주사&접종</option>
	                    	<option value="53">X-ray</option>
	                    </select>
	                  </div>
	                </div>
	              </div>
	              ...
	              <div class="col-sm-4 col-xs-8">
	                <div class="email-search">
	                  <div class="input-group input-search input-group-sm">
	                    <input name="search_text" type="text" placeholder="환자명을 입력하세요" class="form-control input-sm" value="${param.search_text}">
                  ...
                  <table id="mainDataTable" class="table table-striped table-hover table-fw-widget">
                    <thead>
                      <tr class="text-center">
                        ...
                        <th class="text-center">챠트번호</th>
                        <th class="text-center">지원구분</th>
...
<c:forEach var="data" items="${resultMap.list}" varStatus="status">
                      <tr class="text-center">
                        ...
                        <td>${data.chart_no}</td>
                        <c:choose>
	                        <c:when test="${data.view_id eq '51'}">
	                        	<td>검사</td>
	                        </c:when>
	                        <c:when test="${data.view_id eq '70'}">
	                        	<td>주사&접종</td>
	                        </c:when>
	                        <c:when test="${data.view_id eq '53'}">
	                        	<td>X-ray</td>
	                        </c:when>
                        </c:choose>
                        <td>${data.patient_nm}</td>
                        <td> ${data.age}</td>
												<td style="max-width:500px;">${data.content}</td>
