...
try {
	...
	list = noticeBiz.selectNoticeTargetList(paramMap);
...
<c:set var="machineList" value="<%=machineList%>" />
<c:set var="paramMap" value="<%=paramMap%>" />

<head>
		<style>
		.machine-box{ border:1px #eee solid; float:left; padding:5px;}
		</style>
    <script type="text/javascript">
			var targetArr = [];
      function evClickDelete( machineKey ) {
    	  $.ajax({
           url : "${commonviewPath}/notice_target/json/deleteNoticeTarget.jsp",
           type : "POST",
           cache : false,
           dataType : "json",
           data : {"notice_id":${param.notice_id},"target_id":machineKey},
      ...
      function evClickAdd(machineKey, machineName) {
    	  var isDupl = false;
    	  for(var lp0=0; lp0<targetArr.length; lp0++) {
    		  if(machineKey == targetArr[lp0].target_id) {
    			  isDupl = true;
    		  }
    	  }
    	  if(!isDupl) {
    		  var obj = {};
       	  obj.target_id = machineKey;
       	  obj.sign_id = machineName;
       		targetArr.push(obj);
    	  }
    	  showMachineList()
      }
      
      function evClickRemove(machineKey) {
    	  for(var lp0=0; lp0<targetArr.length; lp0++) {
    		  if(machineKey == targetArr[lp0].target_id) {
      ...
      <div class="be-content">
        ...
        <div class="main-content container-fluid">
        	...
        	<div class="cf-search-area">
	          <div class="row">
	            <div class="col-sm-12">
                <form id="cfMainForm" name="mainForm" action="editView.jsp">
...
<c:forEach var="data" items="${machineList}" varStatus="status">
                      <tr class="text-center">
                        ...
                        <td>${data.position}</td>
                        <td>
                          <button class="btn btn-primary" onclick="evClickAdd(${data.machine_id}, '${data.sign_id}')">추가</button>
                        </td>
                      </tr>
</c:forEach>
                    </tbody>
...
<c:forEach var="data" items="${list}" varStatus="status">
                      <tr class="text-center">
                        ...
                        <td>${data.position}</td>
                        <td>
                          <button class="btn btn-danger" onclick="evClickDelete(${data.machine_id})">삭제</button>
                        </td>
                      </tr>
</c:forEach>
                    </tbody>
