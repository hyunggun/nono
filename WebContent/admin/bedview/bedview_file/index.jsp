...
try {
	...
	fileList = bedviewBiz.selectBedviewFileList(paramMap);
} catch(Exception ex) {
    ex.printStackTrace();
}
%>
...
<head>
    <script type="text/javascript">
      ...
      function evClickDelete( fileKey ) {
    	  $.ajax({
            url : "${bedviewPath}/bedview_file/json/deleteFile.jsp",
            type : "POST",
            cache : false,
            dataType : "json",
            data : {"file_id":fileKey},
...
<body>
      ...
      <div class="be-content">
        <div class="page-head">
          <h2 class="page-head-title">병상용 파일</h2>
                    ...
                    <tbody>
                      <tr class="text-center">
                        ...
                        <td class="text-center" style="border:0; font-weight:bold;">
                        	<img id="loadFile" src="" style="max-width:300px;">
                        </td>
                        <td style="border:0;"> <input name="bedviewImage" type="file" accept="image/*" onchange="readURL(this)"/></td>
                        <td style="border:0;">
                          <button type="submit" class="btn btn-success">저장</button>
                          <button type="button" class="btn btn-warning" onclick="location.href='${treatviewPath}/bedview_file'">목록</button>
                        </td>
                      </tr>
                    </tbody>
                  </table>
...
<c:forEach var="data" items="${fileList}" varStatus="status">
                      <tr class="text-center">
                        ...
                        <td>
                          <button class="btn btn-danger" onclick="evClickDelete(${data.file_id})">삭제</button>
                        </td>
                      </tr>
</c:forEach>
                    </tbody>
