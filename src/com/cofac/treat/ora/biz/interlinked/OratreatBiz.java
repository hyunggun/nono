...
public class OratreatBiz extends OracleBiz {
...
//			resultMap.put("inpatientCount", inpatientCount);
	...
	public HashMap selectTreatCheckPage(HashMap paramMap) {
		HashMap resultMap = new HashMap();
		try {
			Integer count = (Integer) sqlMap.queryForObject("oratreat.selectTreatCheckCount", paramMap);
			List list = sqlMap.queryForList("oratreat.selectTreatCheckList", paramMap );
	...
	public List selectTreatCheckList(HashMap paramMap) {
		List list = null;
		try {
			list = sqlMap.queryForList("oratreat.selectTreatCheckList", paramMap );
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
...
//수술실뷰
 ...
 public Boolean updateTreatFlag( HashMap paramMap ) {
   ...
   try {
     sqlMap.update( "oratreat.updateTreatFlag", paramMap );
     isSuccess = true;
   } catch (Exception e) {
     // TODO: handle exception
     e.printStackTrace();
